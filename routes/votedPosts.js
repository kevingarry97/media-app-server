...
router.get('/voted/post/:id', async (req, res) => {
    const votedPosts = await VotedPost.find({"post._id": req.params.id});
...
router.post('/vote/post', auth, async (req, res) => {
    ...
    let votedPost = await VotedPost.findOne({"post._id": req.body.post_id});
